...
let chapterSort = 'old';
let homeTab = 'leaderboard'; // 'leaderboard', 'news', 'authors-log', 'inspirations', 'animated-series'
...
function checkPromoModal() {
    const hasFollowed = localStorage.getItem('afh_follow_instagram');
    // If user has NOT clicked follow, show the modal
    if (hasFollowed !== 'true') {
        setTimeout(() => {
...
window.closePromoModal = () => {
    ...
    // We do NOT set the flag here, so it opens again next time
};
...
async function loadHomeContent() {
    ...
    } else if (homeTab === 'animated-series') {
        renderAnimatedSeries(c);
    }
}
...
function renderAnimatedSeries(container) {
    container.innerHTML = `
        <div class="animate-in fade-in slide-in-from-bottom-4 duration-500 p-2">
            <div class="relative rounded-2xl overflow-hidden border border-purple-500/30 p-8 text-center space-y-6 magical-section">
                <!-- Background Gradient Effect handled by CSS class .magical-section -->
                
                <h3 class="text-2xl font-black text-white uppercase tracking-widest drop-shadow-lg">
                    A New Animation Series
                </h3>
                
                <p class="text-xs text-purple-100 font-bold uppercase tracking-wider opacity-90">
                    is in the planning phase behind the scenes
                </p>

                <div class="py-4">
                    <p class="text-[10px] text-slate-200 uppercase tracking-widest mb-2">Series will be animated by</p>
                    <div class="inline-block relative">
                         <div class="bg-black text-transparent select-none px-4 py-1 rounded text-lg font-black tracking-widest blur-[2px]">SECRET NAME</div>
                         <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-[8px] text-white/50 font-black uppercase tracking-[0.3em]">CENSORED</span>
                         </div>
                    </div>
                    <p class="text-[10px] text-slate-200 uppercase tracking-widest mt-2">Studios</p>
                </div>

                <div class="bg-black/40 inline-block px-6 py-2 rounded-full border border-white/10">
                    <p class="text-xs font-bold text-white uppercase tracking-widest">
                        Releasing Mid 2027
                    </p>
                    <p class="text-[8px] text-red-400 uppercase tracking-widest mt-1">On YouTube</p>
                </div>

                <div class="pt-4">
                    <a href="https://www.instagram.com/falsehope2877/" target="_blank" class="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-3 rounded-xl font-black text-[10px] text-white uppercase tracking-widest hover:scale-105 transition-transform shadow-lg shadow-purple-500/30">
