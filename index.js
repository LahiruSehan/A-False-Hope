...
async function loadHomeContent() {
    ...
    if (homeTab === 'leaderboard') {
        ...
        c.innerHTML = `<div class="grid grid-cols-2 gap-3">${items.map(img => `<img src="${img}" class="w-full aspect-square object-cover rounded-xl border border-white/5 cursor-pointer" onclick="openLightbox('${img}')">`).join('')}</div>`;
    }
    loadActivePoll();
}
...
async function loadChapters() {
    ...
    container.innerHTML = chapters.map(c => `<div class="glass-panel p-4 rounded-xl border border-white/5 flex justify-between items-center" onclick="openReader(${c.id})"><div class="flex items-center gap-4"><span class="bold-italic text-xl text-purple-400 opacity-20">${c.id}</span><p class="text-[11px] font-black text-white uppercase tracking-widest">Chapter ${c.id}</p></div><div class="flex gap-3">${c.bookmarked ? '<span class="text-xs">🔖</span>' : ''}<span class="text-[9px] font-black text-slate-600">♥ ${c.likes}</span></div></div>`).join('');
}
...
window.openChapterComments = async () => {
    ...
    list.innerHTML = (data || []).map(c => `<div class="flex gap-3"><img src="${c.profiles.avatar_url}" class="w-6 h-6 rounded-full object-cover border border-white/5"><div class="flex-1"><div class="flex justify-between items-center"><p class="text-[8px] font-black text-white">${c.profiles.display_name.toUpperCase()}</p><p class="text-[6px] text-slate-600">${new Date(c.created_at).toLocaleDateString()}</p></div><p class="text-[10px] text-slate-300 mt-1">${c.content}</p></div></div>`).join('') || '<p class="text-center opacity-10 py-10">THE VOID IS SILENT</p>';
...
window.showUserProfile = async (userId) => {
    ...
    content.innerHTML = `<img src="${data.avatar_url}" class="w-20 h-20 rounded-full mx-auto border-2 border-purple-500/20 object-cover"><div><h4 class="text-xs font-black text-white uppercase tracking-widest">${data.display_name}</h4><p class="text-[8px] text-purple-400 font-bold uppercase mt-1">${info.name} • LVL ${data.level}</p></div><div class="text-[10px] text-slate-400 italic px-2">${data.bio || "No echo left behind."}</div><div class="grid grid-cols-2 gap-2 text-[9px] border-t border-white/5 pt-4 text-left"><div class="text-slate-500">AGE</div><div class="text-right text-white">${data.age || '—'}</div><div class="text-slate-500">PAGES</div><div class="text-right text-white">${data.total_pages_read || 0}</div><div class="text-slate-500">STREAK</div><div class="text-right text-white">${data.streak_count || 0}d</div></div>`;
...
async function loadReaders() {
    ...
    if (!data) return;
    container.innerHTML = data.map(r => `<div class="glass-panel p-3 rounded-xl border border-white/5 flex items-center justify-between"><div class="flex items-center gap-3 cursor-pointer" onclick="showUserProfile('${r.id}')"><img src="${r.avatar_url}" class="w-8 h-8 rounded-full object-cover border border-white/5"><p class="text-[10px] font-black text-white uppercase">${r.display_name}</p></div>${r.id !== currentUser.id ? `<button onclick="alert('Signal unstable.')" class="bg-purple-600/10 border border-purple-500/20 px-3 py-1 rounded-lg text-[8px] font-black uppercase text-purple-400">Signal</button>` : ''}</div>`).join('');
}
